...
export default class QuestionnaireResponseDetails extends React.Component<Props,State> {
  ...
  render () {
    let contents = this.state.loading ? <LoadingComponent/> : this.renderTabs(this.state.patient);
    return contents;
  }
...
async populateCareplans() {
  ...
  let { cpr } = this.props.match.params;
  let responses = await this.context.backendApi.GetPatientCareplan(cpr)
  this.setState({
      careplans : responses,
      loading : false
  });
  ...
  renderTabs(patient: PatientDetail) {
    return (
<>
      ...
      <Stack>
        ...
        <BasicTabs 
            ...
            tabContent={[this.renderQuestionnaireResponseTab(),this.renderChartsTab()]}
            />
  ...
  renderQuestionnaireResponseTab(){
    return (
